...
### Playground
The containers have a playground configuration that allows users to experiment with functionality without having to create a configuration repo. This playground is self-contained and changes are not persisted after containers are down'd. The [plalyground compose file](./docker-compose.yaml) runs:
- **mongodb**: Simple mongodb instance
- **configurator_spa** to run the WebUI
...
- **mongodb**: Simple mongodb repo for local dev.
- **configurator_spa** to run the WebUI
...
A typical production deployment would not need the SPA, and would EXIT_AFTER_PROCESSING to run in batch mode.
- **custom_api** with packaged configurations and:
    - **MONGO_CONNECTION_STRING**=*your cloud database*
    - **AUTO_PROCESS**=true
